...
- (void)storeCachedResponse:(NSCachedURLResponse *)cachedResponse forRequest:(NSURLRequest *)request
{
    ...
    [Logger log:@"UC - storing" text:[request URL].absoluteString];
...
- (NSCachedURLResponse *)cachedResponseForRequest:(NSURLRequest *)request
{
    ...
    if (memoryResponse)
    {
        [Logger log:@"UC - inMemory" text:[request URL].absoluteString];
    ...
    @synchronized(self.diskCacheInfo)
    {
        ...
        if ([accesses objectForKey:cacheKey]) // OPTI: Check for cache-hit in a in-memory dictionary before hitting the file system
        {
            ...
            if (diskResponse)
            {
                ...
                if (diskResponse)
                {
                    [Logger log:@"UC - onDisk" text:[request URL].absoluteString];
                    return diskResponse;
                }
            }
        }
    }
    [Logger log:@"UC - none" text:[request URL].absoluteString];
...
- (NSData*)getCachedFile:(NSURL *)url
{
    ...
    NSData* fileData = [manager contentsAtPath:cacheFile];
    return fileData;
}
